...
export default class SidetreeTransactionParser {
  ...
  private async fetchOutput (transactionId: string, outputIndexToFetch: number): Promise<BitcoinOutputModel | undefined> {
    try {
      ...
      if (e instanceof SidetreeError) Logger.error(`Error while trying to get outputIdx: ${outputIndexToFetch} from transaction: ${transactionId}. Error: ${SidetreeError.stringify(e)}`);
      throw e;
    }
  }
