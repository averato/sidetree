...
describe('Compressor', () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    it('should compress and decompress as Buffer correctly.', () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        const inputAsBuffer = Buffer.from(randomJsonStr);
        const compressedBuffer = yield Compressor_1.default.compress(inputAsBuffer);
        const decompressedBuffer = yield Compressor_1.default.decompress(compressedBuffer, 100000);
        expect(decompressedBuffer).toEqual(inputAsBuffer);
    }));
    it('Should throw error if decompressed data exceeds maximum size specified.', (done) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        ...
        for (let i = 0; i < uncompressedBuffer.length; i++) {
            uncompressedBuffer[i] = 49;
        }
        const compressedBuffer = yield Compressor_1.default.compress(uncompressedBuffer);
        const maxAllowedDecompressedSizeInBytes = 100000;
        try {
            yield Compressor_1.default.decompress(compressedBuffer, maxAllowedDecompressedSizeInBytes);
        }
        catch (error) {
            if (error instanceof SidetreeError_1.default &&
                error.code === ErrorCode_1.default.CompressorMaxAllowedDecompressedDataSizeExceeded) {
                let message = error.message;
                message = message.substring(0, message.length - 6);
                const bytesDecompressed = Number.parseInt(message.substring(message.indexOf('at ') + 3), 10);
...
//# sourceMappingURL=Compressor.spec.js.map